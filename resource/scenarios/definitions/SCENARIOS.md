...
## 5. Control-plane — Allowed but flagged for audit
...
## 6. Control-plane — Blocked (request never reaches data-plane)
...
## 7. Control-plane — Error (request outcome = error)
...
## 8. Reference only (not part of standard mix)

| Scenario | Intention | Product context |
|----------|-----------|------------------|
| **_EXAMPLE_SCENARIO_** | **Template/reference** only. Documents every supported YAML option. Excluded from listing and mixed workload; use when authoring new scenarios. | Not for product mix; for scenario authors. **Workload weight: 1.0** (irrelevant when excluded). |
...
|-----|------------------------|--------|
| **data-plane** | All new_claim, claim_status, cancel_claim, update_appointment scenarios | Agent/MCP/tool traces, latency, success/4xx |
