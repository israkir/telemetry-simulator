...
- **`happy_path/`** — Data-plane success flows: one scenario per **workflow × division** (9 workflows × 3 divisions = 27 scenarios). Workflows: `new_claim`, `update_appointment`, `claim_status`, `cancel_claim`, `upload_documents`, `choose_insurance`, `buy_insurance`, `cancel_product`, `pay`. Divisions: phone, electronics, appliances.
...
- **Why use it:** Weights let you shape the mix to look realistic—e.g. high weight for the main happy path (e.g. 9.0 for `new_claim_phone`), lower for other happy paths and multi-turn (1.0–6.0), and much lower for rare events like policy errors or blocks (0.08–0.45)—so generated traffic resembles production.
...
| --------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ----- |
| **new_claim_phone**         | Phone      | **Primary happy path.** User starts a phone claim; agent runs planner → task → tool recommendation → `new_claim` → response. One turn, one session, one random conversation sample per run. | Bulk of “create claim” traffic. Use for baseline latency and success-rate expectations. **Workload weight: 9.0** (highest). | data-plane, happy-path |
...
**Why higher-latency condition attributes appear on a2a.orchestrate:** The condition that triggered higher latency (e.g. `peak_hours`, `post_long_weekend`, `claim_status_output`, `zip_code`) is recorded as span attributes on **gentoro.a2a.orchestrate** only (same span as `gentoro.orchestration.duration_ms`). This supports **Filter** (e.g. "show me all orchestration traces where higher latency was due to peak_hours") and **Correlate** (e.g. "this orchestration had higher latency because of peak_hours and post_long_weekend").

**Peak-hours MCP latency by division:** To compare MCP latency across divisions (phone, electronics, appliances) at peak hours for the configured agent, use the three peak-hours scenarios: **generic_higher_latency_peak_hours** (phone), **new_claim_electronics_higher_latency_peak_hours** (electronics), **new_claim_appliances_higher_latency_peak_hours** (appliances). Each emits `peak_hours` (and related) on a2a.orchestrate; MCP latency is on spans (`tool.latency_ms`, `orchestration.duration_ms`); division is implied by `gentoro.mcp.server.uuid`; `gentoro.a2a.agent.target.id` is on the root span.


| Scenario                                           | Division    | Intention                                                                                                                                                                                                | Product context                                                                                    | Tags |
| -------------------------------------------------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- | ----- |
| **claim_status_phone_higher_latency**              | Phone       | `claim_status` is slower when the status is “technician on-route to scheduled appointment.” Each run emits two traces: one normal reply, one technician-on-route style; condition is on span attributes. | Represents status checks that hit a slower backend path (e.g. live ETA). **Filter:** e.g. a2a.orchestrate traces where higher latency was due to claim_status_output. **Correlate:** condition on orchestration span for that request. **Workload weight: 1.0–1.2.** | data-plane, higher-latency, claim-status |
| **update_appointment_phone_higher_latency**        | Phone       | `update_appointment` has higher latency when scheduling in **zip 90210**. Condition (e.g. zip_code) is in the scenario so traces can be filtered.                                                        | Tests latency by “region” or zip for scheduling. **Filter:** e.g. a2a.orchestrate traces where higher latency was due to zip_code. **Correlate:** condition on orchestration span for that request. **Workload weight: 1.0–1.2.**                         | data-plane, higher-latency, scheduling |
| **update_appointment_electronics_higher_latency**  | Electronics | Same as above for **electronics** (TV, laptop, etc.) in **zip 10001**.                                                                                                                                   | Same intention as phone variant, different division. **Filter:** e.g. by zip_code on a2a.orchestrate. **Correlate:** condition on orchestration span. **Workload weight: 1.0–1.2.**                     | data-plane, higher-latency, scheduling, electronics |
| **cancel_claim_appliances_higher_latency**         | Appliances  | `cancel_claim` is slower when the claim has **an appointment already scheduled** (e.g. cancelling a booked visit). Condition (e.g. `appointment_scheduled: true`) is on span attributes.                 | Represents “cancel + release appointment” backend path. **Filter:** e.g. a2a.orchestrate traces where higher latency was due to appointment_scheduled. **Correlate:** condition on orchestration span for that request. **Workload weight: 1.0–1.2.**                  | data-plane, higher-latency, cancel-claim, appliances |
| **generic_higher_latency_peak_hours**              | Phone       | Tool calls are slower during **peak hours** (e.g. weekdays 9am–2pm PT) or **post long weekend**. Condition is defined in the scenario.                                                                   | Tests time-of-day or calendar-based latency behavior. **Filter:** e.g. a2a.orchestrate traces where higher latency was due to peak_hours or post_long_weekend. **Correlate:** condition on orchestration span for that request. **Workload weight: 1.0–1.2.**                    | data-plane, higher-latency, peak-hours |
| **new_claim_appliances_higher_latency_peak_hours** | Appliances  | Same peak-hours / post–long-weekend condition for **new_claim** in appliances (dishwasher, fridge, washer).                                                                                              | Same intention as generic peak-hours, applied to new-claim appliances. **Filter:** e.g. by peak_hours or post_long_weekend on a2a.orchestrate. **Correlate:** condition on orchestration span. **Workload weight: 1.0–1.2.**   | data-plane, higher-latency, peak-hours, appliances |
| **new_claim_electronics_higher_latency_peak_hours** | Electronics | Same peak-hours / post–long-weekend condition for **new_claim** in electronics (TV, laptop, gaming).                                                                                                   | Enables **MCP latency comparison across divisions** at peak hours (phone, electronics, appliances) for the same agent. **Filter:** by peak_hours on a2a.orchestrate; division via gentoro.mcp.server.uuid. **Workload weight: 1.0.** | data-plane, higher-latency, peak-hours, electronics |
...
| Scenario                                            | Division    | Intention                                                                                                                                                                                                | Product context                                                                                           | Tags |
| --------------------------------------------------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | ----- |
| **new_claim_appliances_tool_4xx_invalid_params**    | Appliances  | Agent calls `new_claim` with **wrong or missing parameters** (e.g. date of loss in wrong format, missing required field). One MCP attempt gets `invalid_arguments` and 4xx.                              | Tests validation errors on claim creation (date format, required fields). **Workload weight: 0.45.**       | data-plane, 4xx, invalid-params, entity-format, appliances |
| **new_claim_electronics_tool_4xx_invalid_params**   | Electronics | Same as above for **electronics** (TV, laptop, etc.): wrong date/format or missing fields → 4xx.                                                                                                         | Same intention, electronics division. **Workload weight: 0.45.**                                           | data-plane, 4xx, invalid-params, entity-format, electronics |
| **claim_status_phone_tool_4xx_invalid_params**      | Phone       | Agent calls `claim_status` with **malformed claim ID** (e.g. "8842" instead of "PH-8842"). Backend returns 4xx; exception reflects invalid claim ID format.                                              | Tests “user gave only the number” or wrong prefix for status checks. **Workload weight: 0.45.**            | data-plane, 4xx, invalid-params, claim-id-format, phone |
| **claim_status_appliances_tool_4xx_invalid_params** | Appliances  | Same as above for **appliances**: claim ID without division prefix (e.g. "7733" vs "HA-7733") → 4xx.                                                                                                     | Same intention, appliances. **Workload weight: 0.45.**                                                     | data-plane, 4xx, invalid-params, claim-id-format, appliances |
| **claim_status_electronics_tool_4xx_invalid_params** | Electronics | Same as above for **electronics**: claim ID without division prefix (e.g. "8821" vs "EL-8821") → 4xx.                                                                                                    | Same intention, electronics. Division-agnostic 4xx coverage for claim_status. **Workload weight: 0.45.**   | data-plane, 4xx, invalid-params, claim-id-format, electronics |
| **cancel_claim_phone_tool_4xx_invalid_params**       | Phone       | Agent calls `cancel_claim` with **invalid or missing reason**. Backend expects a reason from an allowed set; user gives free text or empty → 4xx.                                                         | Same as appliances; division-agnostic 4xx coverage for cancel_claim. **Workload weight: 0.45.**            | data-plane, 4xx, invalid-params, cancel-reason, cancel-claim, phone |
| **cancel_claim_appliances_tool_4xx_invalid_params** | Appliances  | Agent calls `cancel_claim` with **invalid or missing reason**. Backend expects a reason from an allowed set (e.g. customer_requested, duplicate, no_longer_needed); user gives free text or empty → 4xx. | Tests validation of cancel reason (different 4xx context than claim ID format). **Workload weight: 0.45.** | data-plane, 4xx, invalid-params, cancel-reason, cancel-claim, appliances |
| **cancel_claim_electronics_tool_4xx_invalid_params** | Electronics | Same as above for **electronics**: invalid or missing cancel reason → 4xx.                                                                                                                                | Same intention, electronics. Division-agnostic 4xx coverage for cancel_claim. **Workload weight: 0.45.**    | data-plane, 4xx, invalid-params, cancel-reason, cancel-claim, electronics |
| **cancel_product_phone_tool_4xx_invalid_params**       | Phone       | Agent calls `cancel_product` with **invalid or missing policy_id** (e.g. empty or malformed). Backend returns 4xx; exception reflects invalid policy_id.                                                      | Division-agnostic 4xx coverage for cancel_product. **Workload weight: 0.45.**                              | data-plane, 4xx, invalid-params, cancel-product, phone |
| **cancel_product_appliances_tool_4xx_invalid_params** | Appliances  | Same as above for **appliances**: invalid or missing policy_id → 4xx.                                                                                                                                    | Same intention, appliances. **Workload weight: 0.45.**                                                       | data-plane, 4xx, invalid-params, cancel-product, appliances |
| **cancel_product_electronics_tool_4xx_invalid_params** | Electronics | Same as above for **electronics**: invalid or missing policy_id → 4xx.                                                                                                                                     | Same intention, electronics. **Workload weight: 0.45.**                                                     | data-plane, 4xx, invalid-params, cancel-product, electronics |
...
- **Ungrounded-summarization scenario** (claim_status_ungrounded_summarization): The agent calls the **correct** tool (e.g. `claim_status`); the tool **succeeds** and returns product/claim info. **response_compose** is marked fail (goal: ungrounded_response) with UngroundedContentError because the agent's reply contains sentences that do not originate from tool output (retrieval of product info summarized incorrectly). Trace shows correct MCP server/tool and `error.type` from template (unavailable); exception carries the cause for analytics.
...
| **agent_confusion_cancel_product_instead_of_claim**               | Electronics                             | User says **“cancel my claim”** (intent: `cancel_claim`) but agent calls **`cancel_product`**. Trace shows `cancel_product` succeeding; **response_compose** is marked fail (WrongToolError) so wrong-tool traffic is visible in error metrics. | Same input cluster → similar tools (cancel_claim vs cancel_product); agent picks wrong tool. **Workload weight: 0.45.** | data-plane, tool-confusion, agent-confusion, cancel-product, cancel-claim |
| **agent_confusion_new_claim_partial_tools**                       | Phone                                   | User wants to **start a phone claim**; expected flow is planner → task → tools_recommend → **new_claim** → response_compose. Agent runs planner, task, tools_recommend, then response_compose **without calling new_claim**. Trace has no MCP tool execution; response_compose fail. Cause: no context about sequence. | Partial set of tools in new-claim workflow. **Workload weight: 0.45.**                                                 | data-plane, agent-confusion, partial-workflow, new-claim |
| **agent_confusion_new_claim_wrong_tool_order**                    | Phone                                   | User wants to **start a phone claim**; expected flow has **new_claim** as the (only) tool. Agent calls **update_appointment** first, then new_claim. Trace shows two MCP tool executions in that order; response_compose fail. Cause: no context about sequence. | First tool called is not the first in expected sequence. **Workload weight: 0.45.**                                     | data-plane, agent-confusion, partial-workflow, wrong-order, new-claim |
| **agent_confusion_claim_status_ungrounded_summarization**         | Phone                                   | User asks for **claim status**; agent correctly calls `claim_status`; tool succeeds with product/claim info. Agent **summarizes incorrectly** — response contains sentences that don't originate from tool output. response_compose fail (UngroundedContentError). Cause: retrieval of product info summarized incorrectly. | Correct tool, correct MCP; response not grounded in tool output. **Workload weight: 0.45.**                              | data-plane, agent-confusion, ungrounded-response, claim-status, phone |
...
| ----------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ----- |
| **request_blocked_by_policy**                         | Policy **denies** the request (e.g. policy rule). Root: `outcome=blocked`, `block.reason=request_policy`. No data-plane.                                                                                             | Most common “block” type in the mix. **Workload weight: 0.35.**                                              | control-plane |
| **request_blocked_invalid_payload**                   | Request **fails payload validation** (e.g. malformed or invalid structure). Root: `outcome=blocked`, `block.reason=invalid_payload`.                                                                                 | Bad or malformed incoming request. **Workload weight: 0.2.**                                                | control-plane |
| **request_blocked_invalid_payload_multi**             | Same as above but with **multiple validation errors**; payload span has multiple validation error events (path, rule, code).                                                                                         | Same as single invalid payload, with richer error details for debugging. **Workload weight: 0.1.**          | control-plane |
| **request_blocked_rate_limited**                      | Request is **rate limited** early. Root: `outcome=blocked`, `block.reason=rate_limited`. Only root and payload spans (no policy or augmentation).                                                                    | Tests rate-limit visibility and impact. **Workload weight: 0.1.**                                           | control-plane |
| **request_blocked_policy_fail_closed**                | **Policy engine fails** (e.g. timeout); system is **fail-closed** so request is **blocked**. Root: `outcome=blocked`, `block.reason=request_policy`, policy span has ERROR and exception (e.g. PolicyEngineTimeout). | Ensures “policy failure → block” is distinguishable from “policy decided block.” **Workload weight: 0.08.** | control-plane |
| **request_blocked_invalid_context_augment_exception** | **Augmentation step fails** (e.g. context binding error); request blocked with `block.reason=invalid_context`. Augmentation span has failure/exception.                                                              | Tests visibility of context/augmentation failures. **Workload weight: 0.08.**                               | control-plane |
...
| Scenario                             | Intention                                                                                                                                                 | Product context                                                                                   | Tags |
| ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | ----- |
| **request_error_policy_runtime**     | **Policy engine throws** at runtime (e.g. timeout). Request `outcome=error`; policy span has status=ERROR and exception event (e.g. PolicyEngineTimeout). | Distinguishes “policy error” from “policy block” in metrics and traces. **Workload weight: 0.15.** | control-plane |
| **request_error_policy_unavailable** | Same pattern as above but exception is **PolicyEngineUnavailable** (policy service down or unreachable).                                                  | Variant of policy failure for dependency/availability monitoring. **Workload weight: 0.12.**      | control-plane |
...
| **ungrounded-response**                                          | agent_confusion_claim_status_ungrounded_summarization                                                                | Response not grounded in tool output (e.g. summarization error) |
